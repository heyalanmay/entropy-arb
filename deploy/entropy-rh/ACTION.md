...
4. 充值 USDC → 到账后显示为 **USDG**
  ...
  - 建议充 **$50**，与 HL 侧对称（你目前总资金约 $100，两腿各 $50）
...
## 3.6b 一键验证（推荐用脚本，不用手敲）

上面两条 curl 我包成了脚本，填地址就行：

```bash
bash /Users/ylh/WorkBuddy/2026-09-02-23-29-43/deploy/entropy-rh/check-rh.sh 0x你的钱包地址
```

它会直接告诉你：账户注册没、account_index 是多少、key 注册没。没注册时打印注册指引；
注册成功后把 `LIGHTER_ACCOUNT_INDEX` 和 `LIGHTER_API_KEY_INDEX` 两个值直接给你。

> 链上余额（USDG）只读接口查不到，去网页账户页看一眼 ≥ $30 即可。
...
# ⚠️ 第 3.5 步（必读）：当前配置的硬约束——先认清再决定

> 这一节是我**实测代码 + 实时盘口**后得出的结论，不是教科书。读不懂就先照做，但心里要有数。

## A. 引擎 100% 是吃单（taker），maker 费率用不上

我翻了下单代码（`venue_hl.py:189` 是 `tif:"Ioc"`，`venue_lighter.py:287` 是 `ORDER_TYPE_MARKET`），
**两腿全是吃单**。Hyperliquid 的挂单费率是 1.5 bps，但因为是吃单，实际走的是 **4.5 bps taker**。
→ 想靠「挂单降费率」把成本砍下来，**不改代码做不到**。

## B. 真实往返成本 ≈ 13 bps（不是 9）

```
HL 腿  : 4.5 taker ×2 = 9.0 bps
RH 腿  : 0 taker ×2   = 0 bps
价差滑点: HL 半价差 0.33×2 + RH 半价差 1.8×2 ≈ 4.3 bps
────────────────────────────────────
合计    : ≈ 13.3 bps
```

你手动试那单反推 ~12 bps，**和这个公式对上了**，说明成本模型是真的。

## C. 但实测跨所溢价只有 ~6.5 bps 峰值

盘中抓了 1 分钟（51 样本）：`buy_edge` 均值 1.0 / 最高 6.57 bps；`sell_edge` 最高 -2.5 bps。
**最高的那一瞬（6.57）刚好够到成本线（6.6），平均远不够。**

## D. 这意味着什么

- 当前阈值（midline -3.7 / upper 11.4 / lower 4.0）下，bot **实际上几乎从不触发**——
  因为触发要溢价 > 7.7 bps，而实测峰值才 6.57。我之前把 upper 调高是为「别亏钱」，
  但代价是「也赚不到钱」。
- **SNDK 这个组合在当前费率下，几乎无利可图**。调阈值调不出利润，只会调出「触发即亏」或「永不触发」。
- 真正能改变局面的只有两件：**① 采满 24~72h 看有没有盘中尖峰时段**；**② 换品种或改代码（maker / 等回归）**。

## E. 所以现在的顺序

1. **RH 账户照开**（这一步不亏，是另一条腿，迟早要办）
2. **采集挂上**（看全天有没有 >13 bps 的窗口）
3. 数据说话：有窗口 → 设阈值进场；没窗口 → 我们换思路（ANTH 稳态价差 / 改 maker / 换所）

> 别因为「几乎不触发」就手贱调低阈值去硬跑——那只会把「永不触发」变成「触发即亏」。
> 先让数据告诉我们答案。
